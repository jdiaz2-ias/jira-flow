...
func (b *Launcher) Open(ctx context.Context, link string) error {
	...
	if err != nil || u.Scheme != "https" || u.User != nil || !strings.HasSuffix(u.Hostname(), ".atlassian.net") || u.RawQuery != "" || u.Fragment != "" || !strings.HasPrefix(u.Path, "/browse/") {
		return &domain.Error{Kind: domain.InvalidInput, Message: "Invalid issue URL."}
	}
	...
	if b.OS == "linux" && b.Env("DISPLAY") == "" && b.Env("WAYLAND_DISPLAY") == "" {
		return &domain.Error{Kind: domain.Unsupported, Message: "No graphical session; open the URL in your browser: " + link}
	}
	program := ""
	switch b.OS {
	case "darwin":
	...
	case "linux":
		program = "xdg-open"
	default:
		return &domain.Error{Kind: domain.Unsupported, Message: "No compatible launcher; open the URL: " + link}
	}
	...
	if err := b.Run(ctx, program, link); err != nil {
		if ctx.Err() != nil {
			return &domain.Error{Kind: domain.Canceled, Message: "Opening canceled or timed out. URL: " + link}
		}
		return &domain.Error{Kind: domain.Unsupported, Message: "Could not start the browser; open the URL: " + link}
	}
	return nil
}
